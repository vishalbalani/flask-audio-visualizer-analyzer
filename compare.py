...
def calculate_fingerprints(filename):
    ...
    #     #fpcalc_out = str(subprocess.Popen(['./fpcalc.exe -raw -length', str(sample_time), filename])).strip().replace('\\n', '').replace("'", "")
    #     #print(fpcalc_out)
...
def correlation(listx, listy):
    if len(listx) == 0 or len(listy) == 0:
        ...
        # able to get here.
        print('Empty lists cannot be correlated.')
...
def get_max_corr(corr, source, target):
    ...
    if corr[max_corr_index] > threshold:
        # print("First audio file: %s" % (source))
        # print("Second audio file: %s" % (target))
        b='Audio similarity percentage: %.2f%% at offset: %i'% (corr[max_corr_index] * 100.0, max_corr_offset)
        return b
